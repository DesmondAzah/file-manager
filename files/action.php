...
if(isset($_POST["action"]))
{
     if($_POST["action"] == "fetch")
     {
      ...
      if(count($folder) > 0)
      {
       ...
       {
         $link = "http://localhost/health/".dirname($_SERVER['PHP_SELF']);
        $output .= '
         <div class="col-sm-6 col-lg-4 col-xl-3 mg-t-10 directory ">
         <div class="media media-folder">
           <i data-feather="folder"></i>
           <div class="media-body">
             <h6><a href="'.$link.''.$name.'" class="link-02" data-id="'.'/'.$name.'">'.$name.'</a></h6>
     ...
     if($_POST['action'] === "fetchByName"){

         $folderName = dirname($_SERVER['PHP_SELF']).$_POST['fileName'];
          $path = realpath($_SERVER["DOCUMENT_ROOT"].$folderName);
         $folder1 = scandir(($path));
         $output = '<div class="container-fluid"><a href="" class="btn-primary btn-sm" data-id="'.getBackFilePath($folderName).'" id="back"><< <strong>'.$folderName.'</strong></a></div>';
         $output .='<div class="container-fluid">';
         if(count($folder1) > 0)
         {
             foreach($folder1 as $name) {
         ...
         <div class="media media-folder">
           <i data-feather="folder"></i>
           <div class="media-body">
             <h6><a href="' . $link . '' . $name . '" class="link-02" data-id="' .$_POST['fileName'].'/'.$name . '">' . $name . '</a></h6>
             <span>' . (count(scandir($_SERVER["DOCUMENT_ROOT"] . $folderName . '/' . $name)) - 2) . ' files, ' . get_folder_size($_SERVER["DOCUMENT_ROOT"] . $folderName . '/' . $name) . '</span>
           </div>
           <div class="dropdown-file">
             <a href="" class="dropdown-link" data-toggle="dropdown"><i data-feather="more-vertical"></i></a>
             <div class="dropdown-menu dropdown-menu-right">
...
function getBackFilePath($filePath){

    $filePathArray = explode("/",$filePath);
    $myFinalPath = '';
    // echo $filePath;
    for($x = 0 ; $x < sizeof($filePathArray)-1; $x++){
        if( !$filePathArray[$x] == "") {
            $myFinalPath .= "/".$filePathArray[$x] ;
        }
    }
    return $myFinalPath;
}